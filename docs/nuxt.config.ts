...
const prodUrl = process?.env.NUXT_PUBLIC_SITE_URL ?? 'https://bitrix-tools.github.io'
const baseUrl = process?.env.NUXT_PUBLIC_BASE_URL ?? '/b24jssdk'
const canonicalUrl = process?.env.NUXT_PUBLIC_CANONICAL_URL ?? `${prodUrl}${baseUrl}`
const gitUrl = process?.env.NUXT_PUBLIC_GIT_URL ?? 'https://github.com/bitrix-tools/b24jssdk'
