...
::note
**`ESM` — рекомендуемый формат модулей.** `CommonJS` тоже поддерживается — вы можете `require('@bitrix24/b24jssdk')` из CommonJS-проекта (он резолвится в полноценную CJS-сборку с внешними зависимостями — см. note ниже; TypeScript-файлы, запускаемые напрямую через `ts-node` / `tsx`, тоже работают). Для использования через `<script>` в браузере поставляется `UMD`-сборка.
::

::note
И `ESM`, и `CommonJS`-точки входа держат свои зависимости (`axios`, `luxon`, `qs-esm`) **внешними**, поэтому потребитель через `require()` использует копию `axios` из своего `node_modules` (дедуплицированную и покрытую `npm audit`). Бандл `UMD` — используемый только для `<script>` / CDN в браузере — единственный, кто инлайнит зависимости.
::
...
::note
В примерах ниже используется синтаксис `import` из `ESM`. В CommonJS-проекте заменяйте каждый `import { X } from '@bitrix24/b24jssdk'` на `const { X } = require('@bitrix24/b24jssdk')`.
::
