...
### Пагинация `tasks.task.list` (поле запроса ≠ поле ответа) {#paginating-taskstasklist-request-field--response-field}

`tasks.task.list` сортирует и фильтрует по `ID` (верхний регистр), но возвращает каждую задачу с `id` в нижнем регистре. Наведите `idKey` на поле **ответа**, а `cursorIdKey` — на поле **запроса**: курсор тогда читает последний `id` со страницы и запрашивает следующий с `>ID`:
