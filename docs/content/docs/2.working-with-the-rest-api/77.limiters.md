...
### 1. RateLimiter (Rate Limiting) {#1-ratelimiter-rate-limiting}
...
### 2. OperatingLimiter (Operation Time Limiting) {#2-operatinglimiter-operation-time-limiting}
...
### 3. AdaptiveDelayer (Adaptive Delays) {#3-adaptivedelayer-adaptive-delays}
...
### 4. RestrictionManager (Main Coordinator) {#4-restrictionmanager-main-coordinator}
...
### Default parameters for regular tariffs (standard) {#default-parameters-for-regular-tariffs-standard}
...
### Parameters for the Enterprise plan {#parameters-for-the-enterprise-plan}
...
### Parameters for bulk data processing {#parameters-for-bulk-data-processing}
...
## Long-Running Requests & Non-idempotent Calls {#long-running-requests-non-idempotent-calls}
...
### Why it goes wrong by default {#why-it-goes-wrong-by-default}
...
### Targeted use — per-call override {#targeted-use-per-call-override}
...
### Last resort — disable all retries {#last-resort-disable-all-retries}
...
## Customizing Error Classification {#customizing-error-classification}
