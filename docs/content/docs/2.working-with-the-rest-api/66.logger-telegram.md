---
title: Telegram Handler
description: 'Sending logs to Telegram'
category: 'logger'
audited: 2026-05-05
navigation.title: Telegram
links:
  - label: TelegramHandler
    iconName: GitHubIcon
    to: https://github.com/bitrix24/b24jssdk/tree/main/packages/jssdk/src/logger/handler/telegram-handler.ts
  - label: TelegramFormatter
    iconName: GitHubIcon
    to: https://github.com/bitrix24/b24jssdk/tree/main/packages/jssdk/src/logger/formatter/telegram-formatter.ts
---

::warning
We are still updating this page. Some data may be missing here — we will complete it shortly.
::

## Quick Start {#quick-start}

::steps{level="3"}

### Installing and setting up a Telegram bot {#installing-and-setting-up-a-telegram-bot}
```bash
# Create a bot via @BotFather
# Get the token and chat_id
```

### Usage on the server side {#usage-on-the-server-side}
```ts
import { LogLevel, Logger, TelegramHandler } from '@bitrix24/b24jssdk'

// Create a logger with Telegram handler
const $logger = new Logger('my-app')

const telegramHandler = new TelegramHandler(LogLevel.ERROR, {
  botToken: 'YOUR_BOT_TOKEN',
  chatId: 'YOUR_CHAT_ID',
  parseMode: 'HTML', // or 'MarkdownV2'
  disableNotification: false, // Send notifications
  disableWebPagePreview: true // Disable link previews
});

$logger.pushHandler(telegramHandler)

// Now critical errors will be sent to Telegram
$logger.error('Payment service unavailable', {
  service: 'stripe',
  error: 'Connection timeout',
  duration: '30s'
})
```

### Usage on the client side {#usage-on-the-client-side}

:::caution
On the client side, it is not supported due to security requirements.
:::

::
