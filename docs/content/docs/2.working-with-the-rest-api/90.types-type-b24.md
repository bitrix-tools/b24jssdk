...
`callMethod` / `callListMethod` / `fetchListMethod` / `callBatch` / `callBatchByChunk` will be removed in **2.0.0**. Migrate to `actions.v{2,3}.{call,callList,fetchList,batch,batchByChunk}.make(options)`.
::
