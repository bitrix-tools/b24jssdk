...
## SdkError codes raised by the SDK {#sdkerror-codes-raised-by-the-sdk}
...
## REST-side codes that come back as `AjaxError` {#rest-side-codes-that-come-back-as-ajaxerror}
...
This is not exhaustive — Bitrix24 publishes the full method-specific list at [apidocs.bitrix24.ru](https://apidocs.bitrix24.ru/). Anything not listed here passes through as-is on `AjaxError.code`.

## Handling patterns {#handling-patterns}

### Distinguish SDK bugs from portal-side failures {#distinguish-sdk-bugs-from-portal-side-failures}
...
### Use `isSuccess` for non-throwing methods {#use-issuccess-for-non-throwing-methods}
...
### Catch around `for await` for `FetchList` {#catch-around-for-await-for-fetchlist}
...
### Decide what to retry {#decide-what-to-retry}
