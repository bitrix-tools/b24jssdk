...
description: 'Метод для выполнения пакетных запросов с автоматическим разбиением на чанки для любого числа команд. Автоматически делит большие наборы команд на батчи по 50 и выполняет их последовательно. Используйте только массивы кортежей или массивы объектов.'
category: 'actions'
audited: 2026-05-29
...
Используйте `BatchByChunkV2.make()`{lang="ts-type"} для выполнения большого числа запросов REST API с автоматическим пакетированием.
...
// Создаём 150 сделок (будут разделены на 3 чанка по 50 сделок каждый)
...
Метод разбивает все команды на чанки и начинает выполнять их по очереди через [`Batch`{lang="ts-type"}](/docs/working-with-the-rest-api/batch-rest-api-ver2/).
...
### Массовое создание сделок CRM {#bulk-crm-deal-creation}
